...
- **Monitor (Finanzas + Health)**: `/monitor/` is a thin Flask page (`templates/monitor.html`) linking to two pure-static subsites that Frozen-Flask does not render — they're copied into `docs/monitor/` as plain files:
  - **Finanzas** (`docs/monitor/finanzas/`): source lives in `monitor_src/finanzas/` (HTML/CSS/Chart.js, no build step) and is committed to the repo. Its `data.json` is regenerated by `scripts/sync_finanzas_notion.py` (reads Notion via `NOTION_TOKEN` + `NOTION_PAGE_ID_FINANZAS`, optionally encrypts with `DATA_PASSPHRASE`), run on a schedule by `.github/workflows/sync-finanzas.yml`. `freeze.py` copies `monitor_src/finanzas/` → `docs/monitor/finanzas/` after every freeze.
...
| `NOTION_TOKEN` | Notion internal integration secret, shared by both Monitor subsites | Required to sync Finanzas data and to build Health |
| `NOTION_PAGE_ID_FINANZAS` | Notion page ID for the "Monitor"/Finanzas page | Required by `scripts/sync_finanzas_notion.py` |
...
| `DATA_PASSPHRASE` | Passphrase used to client-side-encrypt `monitor_src/finanzas/data.json` | Optional but recommended — without it, Finanzas data is exposed in plain text on GitHub Pages |

Set these before running `python freeze.py` if you want the generated `docs/music/index.html` to contain the live catalog. For Monitor, these are configured as GitHub Actions secrets (`Settings → Secrets and variables → Actions`) consumed by `.github/workflows/sync-finanzas.yml` and `.github/workflows/deploy.yml` — both workflows also need "Read and write permissions" enabled under `Settings → Actions → General → Workflow permissions` so they can push the regenerated `data.json` / `docs/`.
